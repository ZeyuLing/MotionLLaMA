...
from mmotion.utils.task.task_lib import MotionInbetween
from mmotion.utils.typing import SampleList


@METRICS.register_module()
class InbetweenMetric(BaseTMRMetric):
    def __init__(self,
                 tmr_model: Dict,
                 motion_key='motion',
                 joints_key='joints',
                 pred_motion_key='pred_motion',
                 pred_joints_key='pred_joints',
    ...
    def compute_metrics(self, results: list) -> Dict:
        all_not_match = sum(result['inbetween_not_match'] for result in results)
        all_joints = sum([result['joints'] for result in results], [])
        all_pred_joints = sum([result['pred_joints'] for result in results], [])
        ...
        res = {
            "inbetween_fid": fid,
            "inbetween_diversity": diversity,
            "inbetween_diversity_gt": diversity_gt,
            "num_inbetween_samples": num_pred_samples,
            "not_matched_inbetween_sample": all_not_match,
            'inbetween_ade': ade,
            'inbetween_fde': fde
        }
        return res
    ...
    def process(self, data_batch, data_samples: SampleList):
        ...
        for data_sample in data_samples:
            ...
            if task not in [MotionInbetween]:
                continue
            ...
            pred_joints = data_sample.get(self.pred_joints_key, None)
