...
CHAIN_DICT = {
    'smpl': t2m_kinematic_chain,
    24: t2m_kinematic_chain,
    22: t2m_kinematic_chain,
    'smplh': t2m_body_hand_kinematic_chain,
    52: t2m_body_hand_kinematic_chain
}

RAW_OFFSETS_DICT = {
    'smpl': t2m_raw_body_offsets,
    24: t2m_raw_body_offsets,
    22: t2m_raw_body_offsets,
    'smplh': t2m_raw_offsets,
    52: t2m_raw_offsets
}
...
def get_adjacent_matrix(kinematic_chain:List[List[int]]):
    max_index = max(max(chain) for chain in kinematic_chain)

    # 初始化邻接矩阵
    adj_matrix = np.zeros((max_index + 1, max_index + 1), dtype=int)

    for chain in kinematic_chain:
        for i in range(len(chain) - 1):
            adj_matrix[chain[i], chain[i + 1]] = 1
            adj_matrix[chain[i + 1], chain[i]] = 1
    return adj_matrix

t2m_adjacent_matrix = get_adjacent_matrix(t2m_kinematic_chain)
t2m_body_hand_adjacent_matrix = get_adjacent_matrix(t2m_body_hand_kinematic_chain)
