...
from mmotion.core.conventions.cameras.convert_convention import \
    enc_camera_convention  # prevent yapf isort conflict
from mmotion.utils.smpl_utils.demo_utils import get_different_colors
from mmotion.utils.files_io.ffmpeg_utils import images_to_video
from mmotion.utils.files_io.path_utils import check_path_suffix
...
class Axes3dBaseRenderer(object):
    ...
    def _draw_scene(self,
        ...
        if show_floor:
            min_x, max_x = visual_range[0]
            min_y, max_y = visual_range[1]
            min_z = visual_range[2][0]  # Typically ground level
            verts = [[min_x, min_y, min_z], [min_x, max_y, min_z],
                     [max_x, max_y, min_z], [max_x, min_y, min_z]]
            xy_plane = Poly3DCollection([verts])
            xy_plane.set_facecolor((0.5, 0.5, 0.5, 0.5))  # Grey color with transparency
...
class Axes3dJointsRenderer(Axes3dBaseRenderer):
    ...
    def render_kp3d_to_video(
        ...
        if not self.if_frame_updated:
            ...
            image_array = self._export_frames(keypoints_np, resolution,
                                              ...
                                              disable_limbs, return_array, show_axis=show_axis,
    ...
    def _export_frames(self, keypoints_np, resolution, visual_range,
                       frame_names, disable_limbs, return_array, show_axis:bool=False, show_floor:bool=True):
        ...
        for frame_index in range(keypoints_np.shape[0]):
            ...
            fig, ax = \
                self._draw_scene(visual_range=visual_range, axis_len=0.5,
                                 ...
                                 cam_hori_angle=cam_hor, show_axis=show_axis,
                                 show_floor=show_floor)
            plt.axis(show_axis)
            #  draw limbs
            ...
            if frame_names is not None:
                frame_text = str(frame_names[frame_index])

                cv2.putText(
                    resized_mat, frame_text,
                    (resolution[0] // 10, resolution[1] // 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5 * resolution[0] / 500,
                    np.array([0, 0, 0]).astype(np.int32).tolist(), 2)
...
def put_text_with_wrap(image, text, position, font, font_scale, color, thickness, max_width):
    words = text.split(' ')
    x, y = position
    line_height = cv2.getTextSize('Tg', font, font_scale, thickness)[0][1] + 5  # Adjust line height

    current_line = ""
    for word in words:
        test_line = current_line + word + ' '
        if cv2.getTextSize(test_line, font, font_scale, thickness)[0][0] > max_width:
            cv2.putText(image, current_line, (x, y), font, font_scale, color, thickness)
            y += line_height
            current_line = word + ' '
        else:
