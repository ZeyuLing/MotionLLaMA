...
from mmotion.utils.smpl_utils.rotation2xyz import Rotation2xyz
from mmotion.utils.smpl_utils.simplify_loc2rot import joints2smpl
...
class npy2obj:
    def __init__(self, npy_path, sample_idx, rep_idx, device=0, cuda=True):
        ...
        if isinstance(self.motions, np.ndarray):
            self.motions = rearrange(self.motions, 't j c -> j c t')
            self.motions = {
                'motion': self.motions[None, ...],
                'num_samples': 1,
                'lengths': [self.motions.shape[-1]],
                'text': ['']
            }
        ...
        if self.nfeats == 3:
            ...
            motion_tensor, opt_dict = self.j2s.joint2smpl(
                self.motions['motion'][self.absl_idx].transpose(2, 0, 1))  # [nframes, njoints, 3]
