...
from mmotion.utils.bvh.bvh_animation import children_list, descendants_mask, positions_global, transforms_global, \
    skin
from mmotion.utils.bvh.bvh_quaternions import Quaternions
import mmotion.utils.bvh.bvh_animation_structure as AnimationStructure
import mmotion.utils.bvh.bvh_animation as Animation
...
class JacobianInverseKinematics:
    ...
    def __call__(self, descendants=None, gamma=1.0):
        ...
        if self.descendants is None:
            self.descendants = descendants_mask(self.animation.parents)
        ...
        if not self.silent:
            gp = positions_global(self.animation)
        ...
        for i in range(self.iterations):
            ...
            gt = transforms_global(self.animation)
            gp = gt[:, :, :, 3]
...
class ICP:
    ...
    def __call__(self, descendants=None, maxjoints=4, gamma=1.0, transpose=False):
        ...
        if not self.silent:
            curr = skin(self.animation, self.rest, self.vweights, self.mesh, maxjoints=maxjoints)
        ...
        for i in range(self.iterations):
            ...
            if not self.silent:
                curr = skin(self.animation, self.rest, self.vweights, self.mesh)
                error = np.mean(np.sqrt(np.sum((curr - self.goal) ** 2.0, axis=-1)))
...
class InverseKinematics:
    ...
    def step(self):
        ...
        # regularization term
        loss += 0.5 * self.crit(self.rotations[1:, [3, 7, 12, 16, 20]],
                                self.rotations[:-1, [3, 7, 12, 16, 20]]) + 0.1 * self.crit(self.rotations[1:],
                                                                                           self.rotations[:-1])
        loss.backward()
        self.optimizer.step()
        self.glb = glb
