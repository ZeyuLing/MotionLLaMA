...
from mmotion.utils.bvh.bvh_animation import Animation
...
def load(filename, start=None, end=None, order=None, world=False, need_quater=True) -> Animation:
    """
...
def save_anime(filename, anim, names=None, frametime=1.0 / 24.0, order='zyx', positions=False, mask=None, quater=False):
    """
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(anim.shape[1]):
            if anim.parents[i] == 0:
                t = save_anime_joint(f, anim, names, t, i, order=order, positions=positions)
...
def save_anime_joint(f, anim, names, t, i, order='zyx', positions=False):
    f.write("%sJOINT %s\n" % (t, names[i]))
    f.write("%s{\n" % t)
    t += '\t'
    ...
    for j in range(anim.shape[1]):
        if anim.parents[j] == i:
            t = save_anime_joint(f, anim, names, t, j, order=order, positions=positions)
            end_site = False
...
def save_dict(filename, data, save_positions=False):
