...
from mmotion.core.conventions.segmentation import body_segmentation
from mmotion.renderer.torch3d_renderer import render_runner
from mmotion.renderer.torch3d_renderer.meshes import \
    ParametricMeshes  # noqa: E501
from mmotion.renderer.torch3d_renderer.render_smpl_config import (
    RENDER_CONFIGS,
)
from mmotion.renderer.torch3d_renderer.smpl_renderer import SMPLRenderer
from mmotion.renderer.torch3d_renderer.utils import \
    align_input_to_padded  # noqa: E501
from mmotion.utils.smpl_utils.demo_utils import (
...
from mmotion.utils.files_io.ffmpeg_utils import (
    check_input_path,
    images_to_array,
    prepare_output_path,
    vid_info_reader,
...
from mmotion.utils.render.mesh_utils import save_meshes_as_objs, save_meshes_as_plys
from mmotion.utils.files_io.path_utils import check_path_suffix
from mmotion.registry import MODELS, CAMERAS
    ...
    if body_model is None:
        if body_model_config is not None:
            ...
            if model_path and osp.isdir(model_path):
                ...
                body_model = MODELS.build(body_model_config)
        ...
        if isinstance(palette, torch.Tensor):
            ...
            for person_idx in range(num_person):
                ...
                if palette[person_idx] == 'random':
                    ...
                    raise ValueError(f'Wrong palette string {palette[person_idx]}. '
...
def render_smpl(
    ...
    Args:
        ...
        convention (str, optional): If want to  use an existing convention,
            ...
            (CAMERA_CONVENTION_FACTORY)[motionllama/core/conventions/cameras/
