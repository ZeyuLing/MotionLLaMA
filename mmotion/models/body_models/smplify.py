...
import numpy as np
import torch
from mmengine import OPTIMIZERS

from mmotion.registry import MODELS, CAMERAS
from mmotion.core.conventions.keypoints_mapping import (
    get_keypoint_idx,
...
class SMPLify(object):
    ...
    def __init__(self,
        ...
        self.keypoints2d_mse_loss = MODELS.build(keypoints2d_loss)
        self.keypoints3d_mse_loss = MODELS.build(keypoints3d_loss)
        self.shape_prior_loss = MODELS.build(shape_prior_loss)
        self.joint_prior_loss = MODELS.build(joint_prior_loss)
        self.smooth_loss = MODELS.build(smooth_loss)
        self.pose_prior_loss = MODELS.build(pose_prior_loss)
        self.pose_reg_loss = MODELS.build(pose_reg_loss)
        self.limb_length_loss = MODELS.build(limb_length_loss)
        ...
        if isinstance(body_model, dict):
            self.body_model = MODELS.build(body_model).to(self.device)
        ...
        if camera is not None:
            if isinstance(camera, dict):
                self.camera = CAMERAS.build(camera).to(self.device)
    ...
    def _optimize_stage(self,
        ...
        optimizer = OPTIMIZERS.build(parameters, self.optimizer)
