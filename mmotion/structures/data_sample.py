...
from mmotion.utils.img_utils import all_to_tensor
...
class DataSample(BaseDataElement):
    ...
    META_KEYS = {
        'interhuman_path': 'interhuman_path',
        'motion_path': 'motion_path',
        'caption': 'caption',
        'interactor_caption': 'interactor_caption',
        'union_caption': 'union_caption',
        'text': 'text',
        'caption_path': 'caption_path',
        'union_caption_path': 'union_caption',
        'interactor_caption_path': 'interactor_caption_path',
        'duration': 'duration',
        'audio_duration': 'audio_duration',
        'num_frames': 'num_frames',
        'motion_num_frames': 'motion_num_frames',
        'interactor_motion_num_frames': 'interactor_motion_num_frames',
        'past_motion_num_frames': 'past_motion_num_frames',
        'middle_motion_num_frames': 'middle_motion_num_frames',
        'future_motion_num_frames': 'future_motion_num_frames',
        'audio_num_frames': 'audio_num_frames',
        'music_num_frames': 'music_num_frames',
        'num_joints': 'num_joints',
        'interactor_interhuman_path': 'interactor_interhuman_path',
    ...
    def stack(cls, data_samples: Sequence['DataSample']) -> 'DataSample':
        ...
        stacked at the first dimension if possible. Otherwise, values will be
        ...
        """
        # 1. get all keys (union of all keys across data_samples)
        all_keys = set()
        for data in data_samples:
            all_keys.update(data.keys())

        # 2. get all meta_keys (union of all metainfo_keys)
        all_meta_keys = set()
        for data in data_samples:
            all_meta_keys.update(data.metainfo_keys())

        # 3. stack data
        stacked_data_sample = DataSample()
        for k in all_keys:
            # Collect all values for this key
            values = [data.get(k, None) for data in data_samples]

            # Filter out None values (in case a data_sample doesn't have this key)
            values = [v for v in values if v is not None]

            # 4. Check if all values are of the same type
            if all(isinstance(val, (torch.Tensor)) for val in values):
                # Check if tensors have the same shape
                if all(val.shape == values[0].shape for val in values):
                    # Stack tensors if they have the same shape
                    stacked_value = torch.stack(values)
                else:
                    # Combine into a list if shapes are different
                    stacked_value = values
            else:
                # Combine into a list if the types are not consistent or not Tensor
                stacked_value = values
        ...
        for k in all_meta_keys:
            values = [data.metainfo.get(k, None) for data in data_samples]
            # Filter out None values (in case a data_sample doesn't have this meta_key)
            values = [v for v in values if v is not None]
