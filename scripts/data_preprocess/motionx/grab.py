...
def transform_motions(data):
    ...
    for fId in range(0, frame_number, down_sample):
        ...
        pose = np.concatenate((pose_root, pose_body, pose_left_hand, pose_right_hand, pose_jaw, pose_expression,
                               pose_face_shape, pose_trans, pose_body_shape), axis=1)
...
if __name__ == '__main__':
    ...
    for case_path in tqdm(findAllFile('data/motionhub/motionx/motion_data/smplx_322/raw_grab')):
        ...
        output_motion_path = case_path.replace('raw_grab', 'GRAB').replace('.npz', '.npy')
        output_text_path = case_path.replace('motion_data/smplx_322/raw_grab', 'GRAB_text').replace('.npz', '.txt')
