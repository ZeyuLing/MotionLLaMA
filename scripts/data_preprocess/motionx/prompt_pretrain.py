...
from mmotion.utils import prompt_str2dict_pretrain
...
def prompt_part_motion(
    ...
    for prompt in prompts:
        ...
        try:
            result = prompt_str2dict_pretrain(tokenizer, prompt)
...
if __name__ == '__main__':
    ...
    args.add_argument('--mean', default='data/global_mean_no_rot.npy')
    args.add_argument('--std', default='data/global_std_no_rot.npy')
    args = args.parse_args()
    # make save dirs
    save_root = os.path.join(args.data_root, 'sequence', 'completion')
    ...
    for vec_path in tqdm(glob.glob(os.path.join(args.data_root, 'vecs_joints_52_no_rot', '*.npy'))):
        index = os.path.basename(vec_path).split('.')[0]
        text_path = os.path.join(args.data_root, 'texts', index + '.txt')
